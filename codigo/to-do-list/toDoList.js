...
function montaListas() {
  ...
  if (localStorage.hasOwnProperty("listasCadastradas")) {
    ...
    for (let i = 0; i < listasCadastradas.length; i++) {
      ...
      <input type="checkbox" class="check" id="${j}" />
    </div>`;
        } else {
          strTarefa += `<div class="tarefa">
      <p class="prioridade ${tarefa.prioridade}">${tarefa.ordem}</p>
      <p class="descricao" id="${j}">${tarefa.descricao}</p>
      <input type="checkbox" class="check" id="${j}" />
    </div>`;
        }
      }
...
function riscaTarefa() {
  let listaTela = document.querySelector(".lista-item.visivel");
  listasCadastradas = JSON.parse(localStorage.getItem("listasCadastradas"));
  listaTela.addEventListener("click", (e) => {
    let lista = listasCadastradas[linkAtivo];
    let elClicado = e.target;
    let idClicado = e.target.id;
    if (elClicado.classList.contains("check")) {
      let tarefas = lista.tarefas;
      let checks = listaTela.querySelectorAll(".check");
      checks.forEach((check, idx) => {
        check.addEventListener("click", (e) => {
          let tarefa = tarefas[idClicado];
          let descricao = check.previousElementSibling;
          if (check.checked == true) {
            descricao.classList.add("concluido");
            if (descricao.classList.contains("concluido"))
